...
void countpulse(){
  ...
  //To judge
  balancecar.stopr += rpulse;
  balancecar.stopl += lpulse;
  //Every 5ms into the interruption, the number of pulses superimposed
  balancecar.pulseright += rpulse;
  balancecar.pulseleft += lpulse;
}
/*Angle PD*/
void angleout(){
...
void inter(){
  ...
  if (speedcc >= 8){                                //50ms into the speed loop control
      Outputs = balancecar.speed_PI_control(kp_speed,ki_speed,kd_speed,front,back,setp0);
      speedcc = 0;
  }
    turncount++;
  ...
  balancecar.posture++;
  balancecar.pwma(Outputs,turnoutput,kalmanfilter.angle,kalmanfilter.angle6,turnl,turnr,front,back,kalmanfilter.accelz,IN1M,IN2M,IN3M,IN4M,PWMA,PWMB);//car total PWM output   
...
void loop() {
  ...
  attachPinChangeInterrupt(0, Code_left, CHANGE);
  ...
  if(Serial.available() > 0){//Receive serial(Bluetooth) data   
      ...
      if(mode==0){//Remote control via Bluetooth mode
        switch(motorRun){
        case 0: turnl = 0; turnr = 0; turnoutput = 0;  // control steering and reversing smart car
        ...
        case 2: turnl = 1; turnr = 0; // turnoutput = 0;  // control steering and reversing smart car // turnl = 1; // control smart 2WD balance turn left
                ...
                break;
        case 4: front = 0; back = -50; turnl = 0; turnr = 0; // turnoutput = 0;  // control steering and reversing smart car // back = 50;// control 2WD balance car backwards 
                digitalWrite(RPin, LOW);digitalWrite(GPin, HIGH); digitalWrite(BPin, HIGH); //red led
                break;
        case 3: front = 50; back = 0; turnl = 0; turnr = 0; // turnoutput = 0;  // control steering and reversing smart car //front = -50;// control 2WD balance car go ahead
      ...
      if(mode==2){//Following  mode
        ...
        if(distance<20&&distance>5){
          ...
          digitalWrite(RPin, HIGH);digitalWrite(GPin, LOW); digitalWrite(BPin, HIGH); //green led
           ...
           front = 0; back = 0; turnl = 0; turnr = 0; turnoutput = 0;  // control 2WD balance car stop
           digitalWrite(RPin, LOW);digitalWrite(GPin, HIGH); digitalWrite(BPin, HIGH); //red led
        }
      }
}
